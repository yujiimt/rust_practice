...
pub fn sort<T: Ord + Send>(x: &mut [T], order: &SortOrder) ->
    Result<(), String>{
        match *order{
...
pub fn sort_by<T, F>(x: &mut [T], comparator: &F) ->
    Result<(), String>
    where   T: Send,
...
mod tests{
    ...
    struct Student{
     ...
     impl Student {
         fn new(first_name: &str, last_name: &str, age: u8) ->
         Self{
             Self{
    ...
    fn sort_student_by_age_ascending(){
        ...
        let ryousuke = Student::new("Ryousuke", "Hayashi", 17);

        let mut x = vec![&taro, &hanako, &kyoko, &ryousuke];

        // ソート後の期待値を作成する
        let expected = vec![&hanako, &kyoko, &taro, &ryousuke];
    ...
    fn sort_student_by_name_ascending(){
        ...
        let mut x = vec![&taro, &hanako, &kyoko, &ryousuke];
        let expected = vec![&ryousuke, &kyoko, &hanako, &taro];
        assert_eq!(sort_by(&mut x, &|a,b| a.last_name.cmp(&b.last_name).then_with(|| a.first_name.cmp(&b.first_name))),
        Ok(())
    );
      assert_eq!(x, expected);  
    }
